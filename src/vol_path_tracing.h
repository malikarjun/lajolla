...
Spectrum get_sigma_t_v(const Medium &medium, Vector3 point) {
	return get_sigma_a(medium, point) + get_sigma_s(medium, point);
}


Spectrum next_event_estimation_v(const Scene &scene, Vector3 p, int current_medium_id, PathVertex &vertex,
								 const Vector3 dir_in, int bounces, pcg32_state &rng, bool surface, int x, int y) {
	// First, we sample a point on the light source.
	Vector2 light_uv{next_pcg32_real<Real>(rng), next_pcg32_real<Real>(rng)};
	Real light_w = next_pcg32_real<Real>(rng);
	Real shape_w = next_pcg32_real<Real>(rng);
	...
	while(true) {
		...
		if(shadow_medium_id >= 0) {
			...
			while(true) {
				...
				accum_t = fmin(accum_t + t, next_t);
				if (t < dt) {
					// didn’t hit the surface, so this is a null-scattering event
					Vector3 point = shadow_ray.org + shadow_ray.dir * accum_t;
					Spectrum sigma_t = get_sigma_t_v(medium, point);
	...
	if (max(T_light) > 0) {
		...
		if (surface) {
			...
			pdf_nee = light_pmf(scene, light_id) *
				pdf_point_on_light(light, point_on_light, orig_point, scene) *
				p_trans_nee;
			contrib = T_light * G * f * L / average(pdf_nee);
...
/*		if (debug(x, y)) {
			print(w, "weight value");
			print(pdf_dir, "pdf dir");
			print(contrib, "contribution");
		}*/
		return w * contrib;
	}
	...
	while(true) {
		...
		if (current_medium_id != -1) {
			...
			while(true) {
				...
				accum_t = fmin(accum_t + t, t_hit);
				Vector3 point = ray.org + accum_t * ray.dir;
				Spectrum sigma_t = get_sigma_t_v(medium, point);
...
/*		if (debug(x, y)) {
			print(current_path_throughput, "current throughput in bounce " + std::to_string(bounces));
		}*/
		...
		if (!scatter && t_hit != INF  && is_light(scene.shapes[vertex.shape_id])) {
			...
			if (bounces == 0 ) {
				...
				// TODO: revisit this.
				Spectrum pdf_dir = dir_pdf * multi_trans_pdf * G;
				Spectrum weight = (pdf_dir * pdf_dir) / (pdf_dir * pdf_dir + pdf_nee * pdf_nee);
				radiance += current_path_throughput * emission(vertex, -ray.dir, scene) * weight;
			}
		}
...
/*			if (debug(x, y)) {
				printf("dot product is %f \n", dot(ray.dir, vertex.geometry_normal));
				printf("changing medium from %d to %d\n", prev_medium_id, current_medium_id);
			}*/
			ray = Ray{vertex.position, ray.dir, get_intersection_epsilon(scene), infinity<Real>()};
		...
		if (scatter && current_medium_id != -1) {
			...
			// we don't need to check for `current_medium_id != -1` because in that case scatter is set to false.
			// do next event estimation for every scatter
			Spectrum nee_contrib = next_event_estimation_v(scene, ray.org, current_medium_id, vertex, dir_in,
														   bounces, rng, false, x, y);
			radiance += current_path_throughput * nee_contrib  * sigma_s;
/*			if (debug(x, y)) {
				print(radiance, "NEE radiance before volume scatter in bounce " + std::to_string(bounces));
			}*/
			...
			Spectrum nee_contrib = next_event_estimation_v(scene, ray.org, current_medium_id, vertex,
														 dir_in, bounces, rng, true, x, y);
			radiance += current_path_throughput * nee_contrib;
/*			if (debug(x, y)) {
				print(radiance, "NEE radiance before surface scatter in bounce " + std::to_string(bounces));
			}*/
			// bsdf sampling
			const Material &mat = scene.materials[vertex.material_id];
