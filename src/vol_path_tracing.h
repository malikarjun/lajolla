...
Spectrum next_event_estimation(const Scene &scene, Vector3 p, int current_medium_id, const Vector3 dir_in,
							   int bounces, pcg32_state &rng, int x, int y) {
	...
	while(true) {
		...
		if (!intersects(vertex_)) {
			...
			shadow_medium_id = update_medium_id(shadow_ray, vertex, shadow_medium_id);
	...
	if (max(T_light) > 0) {
		...
		Real pdf_nee = light_pmf(scene, light_id) * pdf_point_on_light(light, point_on_light, orig_point, scene);
...
Spectrum vol_path_tracing_4(const Scene &scene,
	...
	while(true) {
		...
		if (scatter) {
			...
			Spectrum nee_contrib = next_event_estimation(scene, ray.org, current_medium_id, dir_in, bounces, rng,
														 x, y);
